...
app.use(function (req, res, next) {
  ...
  req.opt = { cluster: req.headers.cluster as Cluster | "localhost" };
  next();
});
...
app.get("/dlmm/create", async (req, res) => {
  try {
    ...
    const dlmm = await DLMM.create(req.connect, poolAddress, req.opt);
    return res.status(200).send(safeStringify(dlmm));
  } catch (error) {
    console.log(error);
    return res.status(400).send(error.message ? error.message : error);
  }
});
